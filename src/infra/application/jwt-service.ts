...
import { JwtTokens, VerifyTokensType, TokensMeta, Payload } from "src/features/auth/api/models/jwt.models";
...
export class JwtService {
  ...
  updateUserTokens(userId: string, deviceId: string): JwtTokens {
      ...
      const refreshToken = jwt.sign({ userId, deviceId }, settings.refresh_secret,
        { expiresIn: "20m" }
      );
