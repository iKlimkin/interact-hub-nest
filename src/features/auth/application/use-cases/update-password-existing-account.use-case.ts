  ...
  implements ICommandHandler<UpdatePasswordForExistingAccountCommand>
{
  ...
  async execute(command: UpdatePasswordForExistingAccountCommand): Promise<boolean> {
    try {
      await validateOrRejectModel(command, UpdatePasswordForExistingAccountCommand);
    } catch (error) {
